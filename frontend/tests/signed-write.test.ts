/**
 * Repository-level proof that Sigil contract writes are SIGNED by the
 * connected wallet.
 *
 * Sigil's wallet context (lib/genlayer/wallet.tsx) builds the client with the
 ...
 * window.ethereum fallback. These tests pin that: a provider-backed client, fed
 * into the real writeAndWait, signs through that provider.
 */
...
describe("Sigil writes are signed by the connected wallet provider", () => {
  it("routes writeContract through the injected EIP-1193 provider (correct from)", async () => {
    ...
    const txHash = await client.writeContract({
      address: CONTRACT, functionName: "create_deal", args: [ACCOUNT, "terms"], value: 10n ** 18n,
    });
  ...
  it("the real writeAndWait helper signs through the provider-backed client", async () => {
    const { client, calls } = providerClient();
    void writeAndWait(client, "create_deal", [ACCOUNT, "terms"], 10n ** 18n).catch(() => {});
    await vi.waitFor(
